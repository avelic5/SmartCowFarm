    ...
    <div className="p-6 md:p-8 space-y-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-slate-300 mt-1">Dodjela prava, statusi i audit trag</p>
        </div>
        <div className="flex flex-wrap gap-3">
          ...
          <button className="inline-flex items-center gap-2 rounded-lg bg-linear-to-r from-green-600 to-blue-600 px-5 py-2.5 text-white shadow-md hover:from-green-700 hover:to-blue-700">
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900/50">
        <div className="space-y-3">
          <div className="flex flex-wrap items-center gap-3 text-sm text-gray-700 dark:text-slate-200">
            <span className="inline-flex items-center gap-2 rounded-full bg-green-50 px-3 py-1 font-medium text-green-700 dark:bg-green-400/10 dark:text-green-200">
              <CheckCircle2 className="w-4 h-4" /> {users.filter((u) => u.status === 'aktivan').length} aktivna
            </span>
            <span className="inline-flex items-center gap-2 rounded-full bg-amber-50 px-3 py-1 font-medium text-amber-700 dark:bg-amber-400/10 dark:text-amber-200">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-slate-700">
              {users.map((u) => (
                <tr key={u.email} className="hover:bg-gray-50 dark:hover:bg-slate-800/50">
                  <td className="px-4 py-3 text-gray-900 dark:text-slate-100">{u.ime}</td>
                  <td className="px-4 py-3 text-gray-700 dark:text-slate-200 inline-flex items-center gap-2"><Mail className="w-4 h-4 text-gray-400 dark:text-slate-400" /> {u.email}</td>
                  <td className="px-4 py-3 text-gray-900 dark:text-slate-100">{displayRole(u.uloga)}</td>
                  <td className="px-4 py-3">
                    <span className={`rounded-full px-3 py-1 text-xs font-medium ${u.status === 'aktivan' ? 'bg-green-50 text-green-700 dark:bg-green-400/10 dark:text-green-200' : 'bg-amber-50 text-amber-700 dark:bg-amber-400/10 dark:text-amber-200'}`}>
                      {u.status}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-gray-700 dark:text-slate-200">{u.kreiran}</td>
                  <td className="px-4 py-3 text-right">
                    <div className="inline-flex gap-2">
                      <button className="rounded-lg border border-gray-300 px-3 py-1 text-gray-700 hover:bg-gray-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800/50">Uredi</button>
                      <button className="rounded-lg border border-gray-300 px-3 py-1 text-gray-700 hover:bg-gray-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800/50">Privilegije</button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {showMatrix && (
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900/50">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-slate-100">Role matrix</h3>
            <p className="text-sm text-gray-500 dark:text-slate-400">Pregled dozvola po ulozi</p>
          </div>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-slate-700">
                {matrix.allPerms.map((perm) => (
                  <tr key={perm} className="hover:bg-gray-50 dark:hover:bg-slate-800/50">
                    <td className="px-4 py-3 text-gray-900 dark:text-slate-100">{perm}</td>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900/50">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-slate-100">Uloge i dozvole</h3>
          <p className="text-sm text-gray-500 dark:text-slate-400">Sažetak po ulozi</p>
        </div>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {Object.entries(permissions).map(([role, perms]) => (
            <div key={role} className="rounded-lg border border-gray-200 p-4 bg-gray-50/70 dark:border-slate-700 dark:bg-slate-950/40">
              <p className="font-semibold text-gray-900 dark:text-slate-100">{displayRole(role)}</p>
